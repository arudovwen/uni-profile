<template>
  <div class="font-Avenir overflow-hidden">
    ...
    >
      <div class="overflow-x-auto">
        ...
        >
          ...
          <template #cell-apps="slotProps">
            <div class="flex flex-wrap gap-1">
              <span
                v-for="(app, idx) in slotProps.data.apps"
                :key="idx"
                class="font-[500] text-[#344054]"
              >
                {{
                  `${app}${idx === slotProps.data.apps.length - 1 ? "" : ","}`
                }}
              </span>
              <span
                v-if="!slotProps.data.apps || slotProps.data.apps.length === 0"
    ...
    <ActionModal
      ...
      :text="`Are you sure you want to ${
        confirmModal.action === 'suspend'
          ? 'deactivate access'
          : 'enable access'
      } for ${confirmModal.user?.user}?`"
      :open="confirmModal.open"
      :btnText="`Yes, ${
        confirmModal.action === 'suspend' ? 'Deactivate' : 'Enable'
      }`"
      :loading="actionLoading"
    />

    <!-- Invite Users Modal -->
    <InviteUsersListModal
    ...
    <EditUserAccessModal
      ...
      @success="handleUserAccessUpdated"
    />
  </div>
</template>
...
// Helper function to map app codes to app names
const getAppNames = (appCodes: string[]): string[] => {
  if (!appCodes || appCodes.length === 0) return [];
  return appCodes.map((code) => {
    const app = subApps.value.find((a: any) => a.code === code);
    return app ? app.name : code;
  });
};

// Fetch sub apps and user apps (for app code to name mapping and available apps)
const fetchAppsData = async () => {
  try {
    const [subAppsRes, userAppsRes] = await Promise.all([
      getSubApps({}),
      getUserApps("1", { PageNumber: 1, PageSize: 50 }),
    ]);

    // Store all apps with full details
    if (subAppsRes.data?.data) {
      subApps.value = subAppsRes.data.data;
    }

    // Get owner's app codes from getUserApps
    let ownerAppCodes: string[] = [];
    if (userAppsRes.data?.data) {
      const userAppsData = userAppsRes.data.data.data || userAppsRes.data.data;
      if (Array.isArray(userAppsData)) {
        ownerAppCodes = userAppsData.map((app: any) => app.code);
      }
    }

    // Filter apps to only those the owner has access to (for modals)
    availableApps.value = subApps.value
      .filter((app: any) => ownerAppCodes.includes(app.code))
      .map((app: any) => ({
        id: app.id || app.code,
        name: app.name || app.appName || app.code,
        code: app.code,
      }));
  } catch (err) {
...
const fetchUsers = async (isLoadMore = false) => {
  ...
  try {
    const res = await getOwnerMembers(queryParams);
    const newUsers = res.data.data.map((user: any) => ({
      ...
      apps: getAppNames(user.appCodes || []),
...
const confirmActionHandler = async () => {
  ...
  try {
    await toggleMemberStatus(user.email);
    const actionText = action === "suspend" ? "deactivated" : "enabled";
    toast.success(`User access ${actionText} successfully`);
