    ...
    <div class="relative">
      ...
      >
        <span
          ...
          :class="[
            modelValue && modelValue.length > 0 ? 'text-gray-900' : 'text-gray-500',
            sizeTextClasses,
          ]"
        >
          {{
        ...
        <!-- Clear Button -->
        <button
          v-if="clearable && modelValue && modelValue.length > 0 && !disabled && !isLoading"
          type="button"
            ...
            >
              ...
              <template v-if="filteredItems.length > 0">
                <button
                  ...
                  :key="`${item.code}-${index}`"
                  type="button"
                  class="w-full px-4 py-3 text-left hover:bg-[#F2F4F7] focus:bg-[#F2F4F7] focus:outline-none transition-colors cursor-pointer border-b border-[#F0F0F0] last:border-0 flex items-center gap-3"
                  :class="{ 'bg-[#EFF8FF]': isSelected(item) }"
...
const isSelected = (item: DropdownOption): boolean => {
  if (!props.modelValue) return false;
  return props.modelValue.some((selected) => selected.code === item.code);
};
