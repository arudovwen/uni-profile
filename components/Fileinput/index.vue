    ...
    <div class="filegroup">
      <label :for="name">
        ...
        >
          ...
          <span
            class="file-name flex-none cursor-pointer border-l px-4 border-slate-200  h-full inline-flex items-center bg-slate-100  text-slate-600 darks:text-slate-400 text-base rounded-tr rounded-br font-normal"
            >{{ label }}</span
          >
        </div>
...
<style lang="scss">
.file-control {
  @apply bg-transparent  transition duration-300 ease-in-out border border-slate-200 darks:border-slate-700 focus:ring-1 focus:ring-slate-900 darks:focus:ring-slate-900 focus:outline-none focus:ring-opacity-90 rounded   text-sm pl-3   placeholder:font-normal;
}
.badge-title {
